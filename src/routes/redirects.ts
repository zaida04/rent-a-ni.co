import type { APPLICATION_CONTEXT, ILink, REDIRECT_CREATE, REDIRECT_DELETE, REDIRECT_GET, Req, Res } from "../typings";
...
export default function homeRouter(context: APPLICATION_CONTEXT) {
    return {
        get: async (req: Req<REDIRECT_GET>, res: Res<REDIRECT_GET>) => {
            ...
            const redirect = await context.DATABASE<ILink>("links").where({ nanoId: req.params.nanoId }).first();
        ...
        post: async (req: Req<REDIRECT_CREATE>, res: Res<REDIRECT_CREATE>) => {
            ...
            if (!authRes[0]) return res.status(401).send({ statusCode: 401, error: "BAD_AUTH", message: authRes[1] });
        ...
        del: async (req: Req<REDIRECT_DELETE>, res: Res<REDIRECT_DELETE>) => {
            ...
            if (!redirect) return res.status(404).send({ statusCode: 404, error: "REDIRECT_NOT_FOUND", message: "redirect not found" });
